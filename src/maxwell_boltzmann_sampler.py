"""Maxwell-Boltzmann speed sampler with a cached inverse-CDF lookup table."""
...
class MaxwellBoltzmannLookupSampler:
    """Inverse-transform Maxwell-Boltzmann sampler with a lookup table.

    Supports v^2 or v^3 PDFs, plus plotting utilities.
    ...
    def __init__(
        ...
        mass_u: float,
        temperature: float,
        pdf: str = "Maxwell-Boltzmann-Distribution v3",
        num_bins: int = 1_000_000,
        v_max: float = None,
    ):
        ...
        if self.pdf_type == "Maxwell-Boltzmann-Distribution v2":
            ...
            self.v_mean = np.sqrt(
                (8 * self.k_B * self.temperature) / (np.pi * self.mass)
            )

        elif self.pdf_type == "Maxwell-Boltzmann-Distribution v3":
            self.v_mp = np.sqrt(3.0 * self.k_B * self.temperature / self.mass)
            self.v_mean = (3.0 * np.sqrt(2.0 * np.pi) / 4.0) * np.sqrt(
                self.k_B * self.temperature / self.mass
            )
    ...
    def _mb_pdf(self, v):
        ...
        if self.pdf_type == "Maxwell-Boltzmann-Distribution v2":
            factor = (
                4
                * np.pi
                * (self.mass / (2 * np.pi * self.k_B * self.temperature))
                ** 1.5
            )
            ...
            factor = (
                2.0 * (self.mass / (2.0 * self.k_B * self.temperature)) ** 2.0
            )
            return factor * v**3 * base
        else:
            raise ValueError(
                "pdf must be 'Maxwell-Boltzmann-Distribution v2' or 'Maxwell-Boltzmann-Distribution v3'"
            )
    ...
    def generate_lookup_table(self):
        ...
        self._inv_cdf = interp1d(
            cdf, v_grid, bounds_error=False, fill_value=(0, self.v_max)
        )
        # store for plotting
        self._v_grid = v_grid
        self._pdf = pdf
    ...
    def sample_speeds(self, n: int, v_min: float = 0.0) -> np.ndarray:
        """Sample n speeds from the cached lookup table (build it first)."""
        if self._inv_cdf is None:
            raise RuntimeError(
                "Lookup table not generated. Call generate_lookup_table() first."
            )
    ...
    def calculate_probability_fraction(self):
        """Fraction of the Maxwell-Boltzmann speed distribution below v_max."""
    ...
    def plot_speeds(
        ...
        speeds: np.ndarray,
        bins: int = 50,
        capture_v: float = 92.0,
        inset_frac: float = 0.20,
        ...
        if not hasattr(self, "_pdf"):
            raise RuntimeError(
                "Lookup table data missing. Call generate_lookup_table() first."
            )
        ...
        hist_counts, edges = np.histogram(
            speeds, bins=bins, range=(0, self.v_max), density=False
        )
        N = len(speeds)
        ...
        if mask_above.any():
            ax.bar(
                centers[mask_above],
                hist_density[mask_above],
                width=widths[mask_above],
                align="center",
                alpha=0.65,
                color="C1",
                label="Sampled velocities",
            )

        if mask_below.any():
            ax.bar(
                centers[mask_below],
                hist_density[mask_below],
                width=widths[mask_below],
                align="center",
                alpha=0.65,
                color="C0",
                label=f"Samples ≤ {capture_v:.0f} m/s",
            )

        # Fitting: fit a single amplitude 'a' so model_density = a * pdf_interp(v)
        pdf_interp = interp1d(
            self._v_grid, self._pdf, bounds_error=False, fill_value=0.0
        )
        ...
        if zero_sigma_mask.any():
            ...
            sigma_density[zero_sigma_mask] = 1.0 / (
                N * widths[zero_sigma_mask]
            )
        ...
        if fit_mask.sum() < 3:
            ...
            print(
                "Not enough bins for fitting; plotting theoretical PDF without fit."
            )
            ax.plot(
                self._v_grid,
                self._pdf,
                "r-",
                lw=2,
                label=r"Maxwell-Boltzmann distribution (theory)",
            )
            ax.axvline(
                self.v_mean,
                label=f"Mean velocity {self.v_mean:.0f} m/s",
                ls=":",
                color="b",
            )
            ax.axvline(
                self.v_mp,
                label=f"Most likely velocity {self.v_mp:.0f} m/s",
                ls="-.",
                color="b",
            )
            ax.axvline(
                capture_v,
                label=rf"$v_c$ ≈ {capture_v:.0f} m/s",
                ls=":",
                color="black",
            )
        ...
        try:
            ...
            print(
                f"Fit successful: reduced chi-squared = {red_chisq:.4g} (chi2={chisq:.4g}, dof={dof})"
            )
            ...
            ax.plot(
                ...
                "r-",
                lw=2,
                label=r"Fitted Maxwell-Boltzmann (shape scaled)",
            )
        ...
        if show_inset and capture_v > 0:
            ...
            h = w
            axins = inset_axes(
                ax, width=w, height=h, loc="lower right", borderpad=5
            )
            ...
            if mask_cap.any():
                ...
                if mask_above_inset.any():
                    axins.bar(
                        centers[mask_above_inset],
                        hist_density[mask_above_inset],
                        width=widths[mask_above_inset],
                        align="center",
                        alpha=0.65,
                        color="C1",
                    )
                if mask_below_inset.any():
                    axins.bar(
                        centers[mask_below_inset],
                        hist_density[mask_below_inset],
                        width=widths[mask_below_inset],
                        align="center",
                        alpha=0.65,
                        color="C0",
                    )
            ...
            if pdf_mask.any():
                ...
                try:
                    if "a_fit" in locals() and not np.isnan(a_fit):
                        axins.plot(
                            self._v_grid[pdf_mask],
                            a_fit * self._pdf[pdf_mask],
                            "r-",
                            lw=1.25,
                        )
                    else:
            ...
            axins.axvline(
                self.v_mean if self.v_mean <= cap else cap,
                ls=":",
                color="b",
                lw=0.8,
            )
            axins.axvline(
                self.v_mp if self.v_mp <= cap else cap,
                ls="-.",
                color="b",
                lw=0.8,
            )
            ...
            if pdf_mask.any():
                ...
                if "a_fit" in locals() and not np.isnan(a_fit):
                    y_candidate = max(
                        y_candidate, (a_fit * self._pdf[pdf_mask]).max()
                    )
                else:
...
if __name__ == "__main__":
    sampler = MaxwellBoltzmannLookupSampler(
        ...
        temperature=623,
        v_max=5000,
        pdf="Maxwell-Boltzmann-Distribution v3",
    )
    sampler.generate_lookup_table()
