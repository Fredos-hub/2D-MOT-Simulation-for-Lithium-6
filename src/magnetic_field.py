###################################################################################################################
#Class to handle calculation of magnetic field. Either by interpolation/grid or by calculation for ideal quadrupol#
###################################################################################################################
...
quadrupole_spec = [('gradient', float64),
                   ('offset', float64[:])]
...
class IdealQuadrupoleField:
    ...
    def calculate_magnetic_field(self, simulation_atoms: ECSAtoms, atom_id: np.ndarray) -> None:
        ...
        """
        calculate_ideal_quadrupole_field(self.gradient, self.offset, simulation_atoms, atom_id)
...
class ZeemanField:
    ...
    """
    def __init__(self, slower_length: float, B_0: float, B_bias: float, delta_B: float, delta_B_min: float) -> None:
...
@njit
def calculate_zeeman_max_step_length(simulation_atoms: ECSAtoms, atom_ids: np.ndarray, slower_length: float, delta_B: float, delta_B_min: float) -> None:
    """
...
@njit
def calculate_ideal_quadrupole_field(gradient: float, offset , simulation_atoms: ECSAtoms, atom_id: np.ndarray) -> None:
    """
