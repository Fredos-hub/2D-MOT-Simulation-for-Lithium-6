...
def _calculate_transition_frequency_shift(
                                          ground_state: int, 
                                          excited_state: int, 
                                          absolute_magnetic_field_strength: float,
    ...
    E_excited = g_p * mu_B * excited_mJ[excited_state] * absolute_magnetic_field_strength
    E_ground = g_s * mu_B * ground_mJ[ground_state] * absolute_magnetic_field_strength
...
def _calculate_saturation_parameter(effective_transition_frequency: float, 
        ...
        effective_detuning = (effective_transition_frequency - (laser_beam_frequency - doppler_shift/(2*math.pi) + detuning/(2*math.pi)))*2*math.pi

        # Calculate Rabi frequencies (with a scaling factor from literature).

        rabi_frequency = 2*math.pi * 1e6 * 11.925*4.37* math.sqrt(transition_strength * 0.001*laser_intensity)
