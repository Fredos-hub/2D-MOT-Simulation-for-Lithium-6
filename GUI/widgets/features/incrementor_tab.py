...
class IncrementorTab(QWidget):
    ...
    def _init_ui(self):
        ...
        self.from_waist = QDoubleSpinBox()
        self.from_waist.setDecimals(1)
        self.to_waist = QDoubleSpinBox()
        self.to_waist.setDecimals(1)
        self.step_waist = QDoubleSpinBox()
        self.step_waist.setDecimals(1)
        ...
        self.from_power = QDoubleSpinBox()
        self.from_power.setDecimals(1)
        self.to_power = QDoubleSpinBox()
        self.to_power.setDecimals(1)
        self.step_power = QDoubleSpinBox()
        self.step_power.setDecimals(1)
        ...
        self.from_detune = QDoubleSpinBox()
        self.from_detune.setDecimals(1)
        self.to_detune = QDoubleSpinBox()
        self.to_detune.setDecimals(1)
        self.step_detune = QDoubleSpinBox()
        self.step_detune.setDecimals(1)
    ...
    def _update_enabled(self, idx):
        ...
        for i, grp in enumerate(groups):
            grp.setEnabled(i == sel)
        for i in (1, 2, 3):
            scopes[i].setEnabled(i == sel)
    ...
    def _add_velocity_sweeps(self):
        ...
        axis_kinds = ("vx", "vy", "vz")
        candidates = []
        for f, t, s, kind in zip(froms, tos, steps, axis_kinds, strict=False):
            if s <= 0:
                continue
        ...
        conflicts = [
            c["kind"] for c in candidates if self._conflicts(existing, c)
        ]
        if conflicts:
            QMessageBox.warning(
                self,
    ...
    def _add_laser_sweep(self, idx):
        ...
        if not vals:
            QMessageBox.warning(
                self, "Invalid range", "Check that From ≤ To and Step > 0."
            )
            return
        sweep = {
            "kind": kind,
            "scope": scope_w.currentIndex(),
            "values": vals,
            "from": f,
            "to": t,
            "step": s,
    ...
    def _tag_for(sweep, value):
        ...
        if k == "vy":
            return f"vy{value:.1f}"
        if k == "vz":
            return f"vz{value:.1f}"
    ...
    def _on_generate(self):
        ...
        if not files:
            QMessageBox.warning(
                self, "No files", "Please add at least one JSON file."
            )
            return
        ...
        for path in files:
            ...
            for combo in product(*(s["values"] for s in sweeps)):
                ...
                tags = []
                for sweep, value in zip(sweeps, combo, strict=False):
