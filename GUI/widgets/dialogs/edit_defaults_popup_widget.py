...
class EditDefaultsPopup(QWidget):
    ...
    """
    FILE_MAP = {
        'Trap': 'trap_default.json',
        'Repump': 'repump_default.json'
    }
    ...
    def __init__(self, parent=None):
        ...
        gui_root = Path(__file__).resolve().parents[1]   # adjust if your structure differs
        ...
        self.selector = QLineEdit()  # hidden hack replaced below by combo for clarity
    ...
    def _on_selector_changed(self, txt):
        ...
        try:
            ...
            QMessageBox.critical(self, "Read error", f"Could not read defaults:\n{e}")
            return
    ...
    def _save(self):
        ...
        for key, (chk, edit) in self.fields.items():
            if chk.isChecked():
                txt = edit.text().strip()
                if txt == "":
                    QMessageBox.warning(self, "Empty value", f"Field {key} is empty.")
                    return
                ...
                try:
                    ...
                    QMessageBox.warning(self, "Invalid", f"'{txt}' is not a number for '{key}'.")
                    return
                new_data[key] = val
                changed = True

        if not changed:
            QMessageBox.information(self, "No changes", "No fields were selected for update.")
            return
        ...
        try:
            ...
            QMessageBox.critical(self, "Write error", f"Could not write defaults:\n{e}")
            return

        QMessageBox.information(self, "Saved", f"Defaults saved to {file_path}")
