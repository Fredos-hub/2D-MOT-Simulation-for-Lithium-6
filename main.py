...
def start_gui(qt_argv, args):
    ...
    icon_path = os.path.join(
        os.path.dirname(__file__), "GUI/icons/simulation_logo_5.png"
    )
    pixmap = QPixmap(icon_path)
    screen = app.primaryScreen()
    screen_geom = screen.availableGeometry()
    max_w, max_h = (
        int(screen_geom.width() * 0.9),
        int(screen_geom.height() * 0.9),
    )
    if pixmap.width() > max_w or pixmap.height() > max_h:
        pixmap = pixmap.scaled(
            QSize(max_w, max_h), Qt.KeepAspectRatio, Qt.SmoothTransformation
        )
...
def main():
    ...
    parser.add_argument(
        ...
        default="light",
        choices=["light", "dark"],
        help="Set the application style (default: light)",
    )
    parser.add_argument(
        "--GUI", action="store_true", help="Run the GUI application"
    )
    parser.add_argument("--files", nargs="*", help="List of files to run")
    parser.add_argument(
        "--target-dir",
        type=str,
        default="2D-MOT-Simulation-For-Lithium-6",
        help="Target directory for simulation outputs",
    )
    ...
    if args.GUI:
        ...
        if not args.files:
            print(
                "No files provided. Use --files file1.json file2.json or run with --GUI."
            )
            return
