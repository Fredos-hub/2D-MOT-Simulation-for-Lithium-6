...
HANDEDNESS_LABELS = {-1: "LH ($\\sigma^-$)", 0: "LIN ($\\pi$)", 1: "RH ($\\sigma^+$)"}
...
def plot_integrated_spectrum(
    ...
    if not traps:
        raise ValueError("No trap laser in config to use as frequency reference.")
    ...
    positions = df[["position_x", "position_y", "position_z"]].to_numpy(np.float64)
    velocities = df[["velocity_x", "velocity_y", "velocity_z"]].to_numpy(np.float64)
    ...
    intensity = _gaussian_intensity(positions, np.asarray(origin_m, np.float64),
                                    dvec, radius_mm * 1e-3, wavelength, 1.0)
    inside = intensity > 0.0
    positions, velocities, ground_states = (positions[inside], velocities[inside],
                                            ground_states[inside])

    interaction = build_interaction(interaction_name)
    mag_field = build_magnetic_field(cfg["Magnetic_Fields"]) if use_b_field else None
    ...
    for h in (-1, 0, 1):
        ...
        res = compute_spectrum_scan(positions, velocities, ground_states,
                                    interaction, mag_field, beam, detunings)
        peak = float(res.rates.max())
        y = res.rates / peak if peak > 0.0 else res.rates
        ax.plot(res.detunings_MHz, y, color=POL_COLORS[h],
                label=HANDEDNESS_LABELS[h], lw=1.8)
    ...
    if repumps:
        rep_off = (float(repumps[0]["beam_frequency"]) * 1e6 - trap_freq_Hz) / 1e6
    ...
    ax.set_title(f"Integrated probe spectrum — {csv_path.parent.name}  "
    ...
    if save_path is None:
        save_path = (Path(__file__).parent / "spectra"
